...
The Level 2A Mask (EMITL2AMASK) data product contains mask information generated from L1B Radiance data and the L2A surface and atmospheric modeling step. The V002 mask contains both traditional mask data (which was previously part of the EMITL1BRFL collection inside the L2A_Mask file) along with cloud mask results from a deep learning model Li et al. (2025). In addition, the geolocation of all pixel centers is included as well as the calculation of observation geometry and illumination angles on a pixel-by-pixel basis. Each image line of the data product is also UTC time-tagged.
